A load balancer is a network device that distributes incoming traffic across a group of servers to improve the performance and availability of applications. Load balancers operate at the network layer [[3.OSI Network Layer]] of the OSI model and are responsible for routing traffic to the appropriate server based on a configured set of rules.
